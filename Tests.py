...
def get_sql_chain(db):
    template = """
        You are a chatbot at NASA's Space Apps Hackathon. You are interacting with a participant who is asking you questions about the challenges and relevant technologies.
...
def get_response(user_query: str, db: SQLDatabase, chat_history: list):
    ...
    template = """
        You are a chatbot at NASA's Space Apps Hackathon. You are interacting with competitors who are asking you questions about the challenges and relevant technologies.

        Based on the table schema, user question, and retrieved data, write a concise natural language response. Do not mention that you're retrieving data from a database or running any specific queries, but provide the relevant technical details where applicable.

        <SCHEMA>{schema}</SCHEMA>
        Conversation History: {chat_history}
        User question: {question}
        Data Retrieved: {response}
    """
...
def chatbot_page():
    if "chat_history" not in st.session_state:
        st.session_state.chat_history = [
            AIMessage(content="Hello! Welcome to NASA's Space Apps Hackathon. How can I assist you with our challenges today?"),
        ]
    ...
    if user_query is not None and user_query.strip() != "":
        if st.session_state.db:
            ...
            with st.chat_message("AI"):
                ...
                # except :
                    # st.error("You maybe entered an invalid prompt.")
